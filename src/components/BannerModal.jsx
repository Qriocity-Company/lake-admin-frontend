...
const BannerModal = ({setBannerModal}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${host}/hotel/update-bottom-banner`, {
        method: "POST",
        body: JSON.stringify({
          imgUrl: fileUrl,
          userId: userId,
  ...
  return (
    ...
    <div className='w-11/12 md:w-[43vw] relative flex flex-col p-6 md:p-10 justify-center items-center rounded-xl bg-white shadow-lg'>
      <X className='h-6 w-6 absolute top-4 right-4 cursor-pointer' onClick={()=>setBannerModal(false)}/>
      <h3 className='text-lg md:text-xl font-bold mb-4'>Update Banner Image:</h3>
      <img 
